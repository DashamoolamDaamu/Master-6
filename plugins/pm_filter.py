...
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, create_invite_links, styled_button
...
def _build_filter_buttons(key: str, req: int, offset: int,
    ...
    row1 = [
        styled_button(_lbl("🌐 Language", cur_lang),
            callback_data=f"flt_{req}_{key}_L"),
        styled_button(_lbl("🎬 Quality",  cur_qual),
            callback_data=f"flt_{req}_{key}_Q"),
    ]
    row2 = [
        styled_button(_lbl("📺 Season",  cur_season),
            callback_data=f"flt_{req}_{key}_S"),
        styled_button(_lbl("▶ Episode", cur_ep),
...
def _build_file_buttons(files: list, pre: str, settings: dict) -> list:
    ...
    return [
        [
            styled_button(f.file_name,
                callback_data=f"{pre}#{f.file_id}"),
            styled_button(get_size(f.file_size),
                callback_data=f"{pre}#{f.file_id}"),
        ]
        for f in files
    ]
...
async def _render_filter_page(
    ...
    if total > 0:
        ...
        btn.append([styled_button("📃 0 / 0", callback_data="pages")])
...
async def _send_pm_search_redirect(message):
    ...
    buttons = InlineKeyboardMarkup([[
        styled_button("👇 Join / Open Group", url=PM_SEARCH_GROUP_LINK)
    ]])
...
async def filter_open(bot, query):
    ...
    for opt in options:
        ...
        picker_rows.append([styled_button(
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        keyboard = InlineKeyboardMarkup([
            [styled_button(f"{stat}", callback_data=f"{cb}:{group_id}"),
             styled_button("DELETE", callback_data=f"deletecb:{group_id}")],
            [styled_button("BACK", callback_data="backcb")]
        ])
...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        buttons = [[
            styled_button('ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
            ],[
            styled_button('ʜᴇʟᴘ', callback_data='help'),
            styled_button('ᴀʙᴏᴜᴛ', callback_data='about')
        ], [
             styled_button(f'ᴏᴛᴛ ᴜᴘᴅᴀᴛᴇs​', url='https://t.me/movie_ottupdates'),
             styled_button(f'ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ', url='https://t.me/mn_movies2')
        ],[
            styled_button('🎬 Browse Movies by Genre', callback_data='genre_menu')
        ],[
            styled_button('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
         ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            styled_button('◀️ Pʀᴇᴠ', callback_data='help_page_5'),
            styled_button('1/6', callback_data='pages'),
            styled_button('Nᴇxᴛ ▶️', callback_data='help_page_1')
        ], [
            styled_button('Mᴀɴᴜᴀʟ Fɪʟᴛᴇʀ', callback_data='manuelfilter'),
            styled_button('Aᴜᴛᴏ Fɪʟᴛᴇʀ', callback_data='autofilter')
        ], [
            styled_button('Cᴏɴɴᴇᴄᴛɪᴏɴ', callback_data='coct'),
            styled_button('Exᴛʀᴀ Tʜɪɴɢs', callback_data='extra')
        ], [
            styled_button('Hᴏᴍᴇ', callback_data='start'),
            styled_button('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            styled_button('◀️ Pʀᴇᴠ', callback_data=f'help_page_{prev_page}'),
            styled_button(f'{page + 1}/{total_pages}', callback_data='pages'),
            styled_button('Nᴇxᴛ ▶️', callback_data=f'help_page_{next_page}')
        ], [
            styled_button('Mᴀɴᴜᴀʟ Fɪʟᴛᴇʀ', callback_data='manuelfilter'),
            styled_button('Aᴜᴛᴏ Fɪʟᴛᴇʀ', callback_data='autofilter')
        ], [
        ...
        buttons = [[
            styled_button('ʙᴀᴄᴋ', callback_data='start'),
            styled_button('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
            ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            styled_button('ʙᴀᴄᴋ', callback_data='about'),
            styled_button('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            styled_button('ʙᴀᴄᴋ', callback_data='help'),
            styled_button('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot'),
            styled_button('ʙᴜᴛᴛᴏɴ', callback_data='button')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            styled_button('ʙᴀᴄᴋ', callback_data='help'),
            styled_button('ᴀᴅᴍɪɴ', callback_data='admin'),
            styled_button('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            styled_button('ʙᴀᴄᴋ', callback_data='help'),
            styled_button('♻️', callback_data='rfrsh'),
            styled_button('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        if settings is not None:
            buttons = [
                [
                    styled_button('Filter Button',
                                         callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}'),
                    styled_button('Single' if settings["button"] else 'Double',
                                         callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}')
                ],
                [
                    styled_button('Bot PM', callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}'),
                    styled_button('✅ Yes' if settings["botpm"] else '❌ No',
                                         callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}')
                ],
                [
                    styled_button('File Secure',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}'),
                    styled_button('✅ Yes' if settings["file_secure"] else '❌ No',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}')
                ],
                [
                    styled_button('IMDB', callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}'),
                    styled_button('✅ Yes' if settings["imdb"] else '❌ No',
                                         callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}')
                ],
                [
                    styled_button('Spell Check',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}'),
                    styled_button('✅ Yes' if settings["spell_check"] else '❌ No',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}')
                ],
                [
                    styled_button('Welcome', callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}'),
                    styled_button('✅ Yes' if settings["welcome"] else '❌ No',
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        button = [[
                 styled_button('ENG', callback_data='esp'),
                 styled_button('MAL', callback_data='msp'),
                 styled_button('HIN', callback_data='hsp'),
                 styled_button('TAM', callback_data='tsp')
        ],[
                 styled_button('🔍 ɢᴏᴏɢʟᴇ 🔎', url=f"https://www.google.com/search?q={reqst_gle}")
             ]]
    ...
    btn.append([styled_button(text="✘ ᴄʟᴏsᴇ ✘", callback_data=f'spol#{reqstr1}#close_spellcheck')])
