...
from utils import get_settings, get_size, is_subscribed, save_group_settings, temp, create_invite_links, get_chat_join_link, styled_button
...
async def create_file_buttons(client, sent_message):
    ...
    try:
        ...
        if invite_link:
            buttons.append([styled_button("📢 Join Channel", url=invite_link)])
        buttons.append([styled_button("🔗 View File", url=message_link)])
...
async def checksub_callback(client, callback_query):
    ...
    if await is_subscribed(user_id, client):
        ...
        btn = [[styled_button("🤖 Join Updates Channel", url=url)] for url in links.values()]
        btn.append([styled_button("🔄 Try Again", callback_data=data)])
    ...
    if not await is_subscribed(message.from_user.id, client):
        ...
        if len(message.command) == 2:
            try:
                ...
                btn.append([styled_button("🔄 Try Again", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([styled_button("🔄 Try Again", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
