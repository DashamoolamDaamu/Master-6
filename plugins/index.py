...
from utils import temp, styled_button
import re
...
CANCEL_MARKUP = InlineKeyboardMarkup(
    [[styled_button('Cancel', callback_data='index_cancel')]]
)
...
async def send_for_index(bot, message):
    ...
    if message.from_user.id in ADMINS:
        buttons = [
            [styled_button('Yes', callback_data=f'index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}')],
            [styled_button('Close', callback_data='close_data')],
        ]
    ...
    buttons = [
        [styled_button('Accept Index', callback_data=f'index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}')],
        [styled_button('Reject Index', callback_data=f'index#reject#{chat_id}#{message.id}#{message.from_user.id}')],
    ]
    await bot.send_message(
        LOG_CHANNEL,
