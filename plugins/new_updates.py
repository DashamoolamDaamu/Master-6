...
from utils import get_poster, styled_button
...
def _build_picker_keyboard(
    ...
    if page > 0:
        nav_row.append(styled_button("⬅️ Prev", callback_data=f"gdl_page:{user_id}:{page - 1}"))
    if page < total_pages - 1:
        nav_row.append(styled_button("Next ➡️", callback_data=f"gdl_page:{user_id}:{page + 1}"))
    if nav_row:
        buttons.append(nav_row)

    buttons.append([styled_button("❌ Cancel", callback_data=f"gdl_cancel:{user_id}")])
...
async def getdlink_pick_callback(bot: Client, query) -> None:
    ...
    season_row_1 = [styled_button("🎬 Movie", callback_data=f"gdl_season:{user_id}:0")]
    season_rows  = [season_row_1]
    row: list[InlineKeyboardButton] = []
    for s in range(1, 11):
        row.append(styled_button(f"S{s:02d}", callback_data=f"gdl_season:{user_id}:{s}"))
        if len(row) == 5:
            season_rows.append(row)
            row = []
    if row:
        season_rows.append(row)
    season_rows.append([styled_button("❌ Cancel", callback_data=f"gdl_cancel:{user_id}")])
...
async def getdlink_season_callback(bot: Client, query) -> None:
    ...
    confirm_markup = InlineKeyboardMarkup(
        search_btn_markup.inline_keyboard + [[
            styled_button("✅ Send to Channels", callback_data=f"gdl_confirm:{user_id}"),
            styled_button("❌ Cancel",           callback_data=f"gdl_cancel:{user_id}"),
        ]]
    )
...
def _build_summary_page(
    ...
    if page > 0:
        nav.append(styled_button("⬅️ Prev", callback_data=f"sumpage:{page - 1}"))
    if page < total_pages - 1:
        nav.append(styled_button("Next ➡️", callback_data=f"sumpage:{page + 1}"))
...
async def sendupnow_cmd(bot: Client, message) -> None:
    ...
    markup = InlineKeyboardMarkup([[
        styled_button("✅ Send Now", callback_data="sendupnow:confirm"),
        styled_button("❌ Cancel",   callback_data="sendupnow:cancel"),
    ]])
